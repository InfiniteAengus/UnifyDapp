        ...
        if(index){
            ...
            try {
                ...
                if(category > 0){
                    ...
                    let ask_id = await tncLibMarket.getCategory(category, index, _this.wrapperAddress);
                    ask = await tncLibMarket.getAskBase(ask_id, _this.marketAddress);
                    index = ask_id;

                }else{

                    ask = await tncLibMarket.getAskBase(index, _this.marketAddress);
                }
            ...
            seller = ask.seller;
            royalties = _this.formatNumberString(await tncLibMarket.getRoyalties(ask.erc1155Address[0], ask.id[0], _this.marketAddress), 2);
        }
    ...
    this.getMarketNfts = async function(address, which, category){
        ...
        for(let i = length - 1; i >= stop; i--){
            ...
            if(category > 0){
                ...
                ret = {ask: await tncLibMarket.getAskBase(ask_id, _this.marketAddress), index: i};
