function TncDapp() {
  ...
  this.getBridgedNfts = async function () {
    ...
    for (let i = 0; i < length; i++) {
      ...
      let nftId = await tncLibBridgeOut.getBridgedId(
        job._srcChainId,
        job._srcAddress,
        job._srcId
      );
  ...
  this.render = async function (erc1155, id, address, job, index, srcUri) {
    ...
    try {
      ...
      if (typeof data == "object") {
        data_image =
          typeof data.image != "undefined" && data.image
            ...
            : "";
        data_animation_url =
          typeof data.animation_url != "undefined" && data.animation_url
            ? data.animation_url
                .replace("ipfs://", "https://gateway.ipfs.io/ipfs/")
                .replace("/ipfs/ipfs/", "/ipfs/")
            : "";
        data_audio_url =
          typeof data.audio_url != "undefined" && data.audio_url
            ? data.audio_url
                .replace("ipfs://", "https://gateway.ipfs.io/ipfs/")
                .replace("/ipfs/ipfs/", "/ipfs/")
            : "";
        data_interactive_url =
          typeof data.interactive_url != "undefined" && data.interactive_url
            ? data.interactive_url
                .replace("ipfs://", "https://gateway.ipfs.io/ipfs/")
                .replace("/ipfs/ipfs/", "/ipfs/")
            : "";
        data_name =
          typeof data.name != "undefined" && data.name ? data.name : "";
        data_description =
          typeof data.description != "undefined" && data.description
            ? data.description
            : "";
        data_link =
          typeof data.external_link != "undefined" && data.external_link
            ? data.external_link
            : "";
        data_attributes =
          typeof data.attributes != "undefined" && data.attributes
            ? data.attributes
            : [];
      }
    } catch (e) {
      try {
        let data = await $.getJSON(
          nft.uri
            .toLowerCase()
            .replace("gateway.ipfs.io", "cloudflare-ipfs.com")
        );
    ...
    let tmpl = _this.bridgedTemplate({
      ...
      collectionName:
        meta.name != "n/a"
          ? '<div class="text-truncate" style="font-size: 1.4rem !important;">' +
            meta.name +
            "</div>"
          : '<div class="text-truncate" style="font-size: 1.4rem !important;">' +
            erc1155 +
      ...
      opensea:
        chain_id == "1" || chain_id == "4"
          ? "https://opensea.io/assets/" + erc1155 + "/" + id
          : "collectible.html?collection=" + erc1155 + "&id=" + id,
    });

    $("#bridgedPage").append(tmpl);

    $("#redeemButton" + index).on("click", async function () {
      let fee = await tncLibBridgeOut.getNifFee();
      let balance = web3.utils.toBN(
        web3.utils.toWei(
          "" +
            (await tncLib.balanceOfErc20(
      ...
      if (balance.lt(fee)) {
        _alert(
          "Insufficient NIF funds for fees. Required: " +
            _this.formatNumberString(fee.toString(), 18) +
            " NIF"
        );
        return;
      }

      let allowance = web3.utils.toBN(
  ...
  this.populateInteractive = async function (e) {
    ...
    $("#runNft").on("click", function () {
      let tag =
        '<iframe style="width:100%;height:400px;border:none;" hspace="0" vspace="0" marginHeight="0" marginWidth="0" frameBorder="0" allowtransparency="true" src=' +
        JSON.stringify($("#nftInteractiveUrl").val()) +
      ...
      default:
        $("#gasFunds").html(
          _this.formatNumberString(
            await tncLibBridgeOut.getGasFunds(tncLib.account),
            18
          )
        );
        ...
        $("#nftInteractiveModal").on(
          "show.bs.modal",
          _this.populateInteractive
        );
  ...
  this.accountChangeAlert = function () {
    _alert(
      'Account has been changed. Please <button class="btn btn-primary" onclick="location.reload()">click here</button> to reload this dapp.'
    );
  };

  this.chainChangeAlert = function () {
    _alert(
      'The network has been changed. Please <button class="btn btn-primary" onclick="location.reload()">click here</button> to reload this dapp.'
    );
  };
  ...
  this.startAccountCheck = function () {
    if (window.ethereum) {
      window.ethereum.on("accountsChanged", function (accounts) {
        const _that = _this;
        if (
          accounts.length != _that.prevAccounts.length ||
          accounts[0].toUpperCase() != _that.prevAccounts[0].toUpperCase()
        ) {
  ...
  this.startChainCheck = function () {
    if (window.ethereum) {
      window.ethereum.on("chainChanged", async function (chain) {
        ...
        console.log(
          "chain check: ",
          actualChainId + " != " + _this.prevChainId
        );
...
function run(connected) {
  $(document).ready(async function () {
    ...
    if (
      ...
      ethereum &&
      typeof ethereum.enable != "undefined" &&
      ethereum.enable
    ) {
      ...
      typeof ethereum != "undefined" &&
      ethereum &&
      (typeof ethereum.enable == "undefined" || !ethereum.enable)
    ) {
