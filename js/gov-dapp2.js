function TncDapp() {
    ...
    this.withdrawNif = async function(){
        ...
        let zero   = web3.utils.toBN("0");
        let credit = web3.utils.toBN(await tncLibGov.credit(tncLib.account));

        if(credit.eq(zero)){
            _alert('No funds to withdraw.');
            return;
        }
    ...
    this.updateStakedNifDisplay = async function(){
        ...
        if(typeof tncLibGov != 'undefined'){
            ...
            let zero = web3.utils.toBN("0");
            let _credit = web3.utils.toBN(credit);
            if(_credit.gt(zero)){
                $('#cooldown').css('display', 'inline-block');
            }
            let nif = Number(_this.cleanUpDecimals(_this.formatNumberString(web3.utils.toBN(credit).toString(), 18))).toFixed(4);
            $('#withdrawableNIF').html(nif);
            $('#nifUnstaked').html(nif);

            let accountInfo = await tncLibGov.accountInfo(tncLib.account);

            $('#nifCooldownAvailable').html( new Date(parseInt(accountInfo[3]) * 1000).toUTCString() );
    ...
    $(document).ready(async function(){
        ...
        $('#stakeMaxButton').on('click', async function(){

            let nifBalance = await tncLib.balanceOfErc20Raw(tncLib.nif.options.address, tncLib.account);
